...
class ZobristHashing:
    ...
    def calculate_hash(self, board):
        ...
        for row in board.board:
            for piece in row:
                if piece != 0:
                    h ^= self.zobrist_table[(piece.row, piece.col, type(piece).__name__, piece.color)]
        return h
    ...
    def update_hash(self, h, piece, old_position, new_position):
        ...
        if old_position:
            h ^= self.zobrist_table[(old_position[0], old_position[1], piece_type, piece_color)]
        if new_position:
            h ^= self.zobrist_table[(new_position[0], new_position[1], piece_type, piece_color)]
        return h